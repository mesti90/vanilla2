...
rule make_reference_gbk:
	...
	shell:
		"""
		python scripts/genbank_annot.py \
			-s {input} \
			--threads {threads} \
			--subthreads 2 \
			--emboss-container {config[containers][emboss]} \
			--diamond-container {config[containers][diamond]} \
			--bind "/node8_R10,/node10_R10,/scratch"
...
rule gbk_to_gff:
	input:
		gbk = "References/{reference}.gbk"
	output:
		gff = "References/{reference}.gff"
	script:
		"../scripts/gbk2gff.py"

rule snpeff_prepare_db:
	input:
		gbk=lambda wc: f"References/{SAMPLE_REF_NAME[wc.sample]}.gbk"
	output:
		config="work/snpeff_db/{sample}/snpEff.config",
		gbk = "work/snpeff_db/{sample}/ref/genes.gbk"
	params:
		refname="ref"
	script:
		"../scripts/snpeff_prepare_db.py"

rule snpeff_build_db:
	input:
		config=rules.snpeff_prepare_db.output.config
	output:
		done="work/snpeff_db/{sample}/.done"
	container:
		config["containers"]["snpeff"]
	params:
		refname=rules.snpeff_prepare_db.params.refname
	shell:
		r"""
		set -euo pipefail

		BASE=$(dirname {input.config})
		snpEff build -c {input.config} -genbank -v {params.refname} -q
